...
public class SnakeMediumRenderer extends JPanel {
	...
	protected void paintComponent(Graphics g) {
		...
		SnakeMedium snake = SnakeMedium.snake;
		g.setColor(Color.BLUE);
		
		//For every coordinate of the snake parts draw a rectangle to the scale of the snake
		for (Point point : snake.snakeParts)
		{
			g.fillRect(point.x * SnakeMedium.SCALE, point.y * SnakeMedium.SCALE, SnakeMedium.SCALE, SnakeMedium.SCALE );
		}
		
		g.fillRect(snake.head.x * SnakeMedium.SCALE, snake.head.y * Snake.SCALE, SnakeMedium.SCALE, SnakeMedium.SCALE );
		
		//Create a red rectangle to the scale of the cherry wherever it spawns
		g.setColor(Color.RED);
		if (snake.begin)
			g.fillRect(snake.cherry.x * SnakeMedium.SCALE, snake.cherry.y * SnakeMedium.SCALE, SnakeMedium.SCALE, SnakeMedium.SCALE );
		
		//Display the score, length, time, and high score at the top of the game screen
		String string = "Score:" + snake.score + ", Length: " + snake.tailLength + ", Time: " + snake.time/30 +", HighScore: "+snake.highscore;
