...
public class SnakeHardRenderer extends JPanel {
	...
	protected void paintComponent(Graphics g) {
		...
		SnakeHard snake = SnakeHard.snake;
		g.setColor(Color.BLUE);
		
		//For every coordinate of the snake parts draw a rectangle to the scale of the snake
		for (Point point : snake.snakeParts)
		{
			g.fillRect(point.x * SnakeHard.SCALE, point.y * SnakeHard.SCALE, SnakeHard.SCALE, SnakeHard.SCALE );
		}
		
		g.fillRect(snake.head.x * SnakeHard.SCALE, snake.head.y * Snake.SCALE, SnakeHard.SCALE, SnakeHard.SCALE );
		
		//Create a red rectangle to the scale of the cherry wherever it spawns
		g.setColor(Color.RED);
		if (snake.begin)
			g.fillRect(snake.cherry.x * SnakeHard.SCALE, snake.cherry.y * SnakeHard.SCALE, SnakeHard.SCALE, SnakeHard.SCALE );
		
		//Display the score, length, time, and high score at the top of the game screen
		String string = "Score:" + snake.score + ", Length: " + snake.tailLength + ", Time: " + snake.time/37 +", HighScore: "+snake.highscore;
